...
GS편의점 내일배송이 두 가지로 바뀌었다. 배송사가 딜리박스에서 CJ대한통운으로 넘어갔고,
권역이 수도권 일부에서 전국으로 열렸다. 외부 서비스의 변화라 코드가 알아서 따라가지 않는다.

### 배송사 코드
- 내일배송 발송처리 코드를 `CJGLS`로 교체 — 국내택배와 같은 값이 됐지만 상수는 나눠 뒀다.
  내일배송 쪽은 설정으로 덮어쓸 수 있는 값이고, 다시 갈라질 여지가 있다
- 옛 코드를 폐기 목록에 등록해 저장값이 남아 있으면 기본값으로 되돌린다. 로컬·서버 DB가
  따로 있어 한쪽만 고치면 다른 쪽이 조용히 옛 코드로 계속 발송한다. 네이버가 옛 코드를
  거절하지 않기 때문에 에러도 나지 않고, 구매자가 운송장을 추적하지 못하는 것으로만 드러난다

### 지역 판별
- 허용 목록에서 제외 목록으로 뒤집었다. 이제 물어야 할 것은 "어디가 되는가"가 아니라
  "어디가 안 되는가"다
- 제주는 우편번호 범위(63000~63644)로 판별한다. 주소 문자열만 보면 다른 지역의 도로명에
  걸린다. 우편번호가 비어 있을 때만 주소 키워드로 넘어간다
- 배로 들어가는 섬은 지명으로 걸러낸다. 완전한 목록이 아니며 예약이 거절된 지역이 나오면
  추가한다. 다리로 이어진 강화도·영흥도·거제도는 넣지 않았다
- 산간 지역은 제외하지 않았다. 쓸 만한 판별 기준이 없어 과하게 막을 위험이 더 크다

### 기본 배송 유형
- 신규 주문은 국내택배로 고정. 전국이 열려 거의 모든 주문이 내일배송 가능이 되는데,
  자동으로 태우면 요금을 따져보는 단계가 사라진다
- 동기화의 갱신 경로는 가능 여부만 새로 계산하고 선택은 건드리지 않는다. 2분 폴링이
  사용자가 고른 내일배송을 국내택배로 되돌리는 것을 막기 위함
- 기동 시 한 번 발송대기·실패 주문을 새 기준으로 재계산한다. 예약이 끝난 주문과 방문수령
  선택은 그대로 둔다 — 접수된 내용과 화면이 어긋나기 때문

### 산출물
- 신규: `src/lib/db/migrations.ts` (데이터 마이그레이션 + 테이블 존재 가드)
- 단위 테스트 11건 (지역 판별 8건, 마이그레이션 3건)
