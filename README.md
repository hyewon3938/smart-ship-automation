...
```
┌─────────────────────────────┐      ┌──────────────────────────────────┐
│         로컬 Mac             │      │     Oracle Cloud VM (서버)        │
│                             │      │                                  │
│  Next.js (개발 모드)         │      │  Next.js (프로덕션, PM2)          │
│  ├─ Playwright headed        │      │  ├─ Playwright headless           │
│  │  └─ GS택배 예약 자동화    │ ───> │  │  └─ 운송장번호 스크래핑         │
│  └─ 쿠키/결과 자동 동기화    │      │  ├─ 네이버 발송처리 자동화         │
│                             │      │  ├─ SQLite DB                     │
│  http://localhost:3000       │      │  └─ Caddy (HTTPS)                │
└─────────────────────────────┘      │     ship.leecommit.kr             │
                                     └──────────────────────────────────┘
```
